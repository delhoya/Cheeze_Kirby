...
+ 이미지 데이터 처리 (CNN) 
  ...
  + CNN 적용 
    ...
    + filter의 weight,bias -> feature를 만듬 (@@@ 2개의 필터 / 2개의 피쳐맵 @@@) 
      + Conv 2D 
      + ex) 초기 커널 -> 컬러 이미지 3개 (5개의 커널 할당) (Ww,Hh,3) (5개 피처맵)
      + ex) 5개의 피처맵 -> 4개의 피처맵 (4개의 커널 할당) ( Fw , Fh , 5) (입력은 같음 / 2D conv라고 불리는 이유) 
    + 아웃풋 피처맵의 크기 : (입력사이즈 - 필터사이즈)/스트라이드 + 1    
    ...
    + 레이어가 깊어질수록 엣지 - 오브젝트 파트 - 오브젝트 로 레이어별로 보는 영역이 달라짐. (넓은영역) 
      + 레이어가 깊어질수록 필터의 개수가 많아져야 (보는 오브젝트가 많아지지)
