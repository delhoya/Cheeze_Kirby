...
+ 머신러닝
  ...
  + SVD : 
    + 특이값 분해 :  m x n 크기의 데이터 행렬 A를 분해하는 걸 말합니다.
    + 고유값 분해 : 변환 후 벡터가 변환 전 벡터에서 얼마나 확대, 축소를 가했는지를 파악하기 위함이다.
    + https://techblog-history-younghunjo1.tistory.com/66  
  + PCA : 
    + PCA : 입력 데이터들의 공분산 행렬(covariance matrix)에 대한 고유값분해(eigendecomposition)
    + 데이터의 분포에 대한 주성분 즉, 최적의 Feature 조합을 찾기 위한 방법
