...
+ 머신러닝
  ...
  + HMM
  + SVM
    + SVM의 목적은 마진을 최대화하는 경계면을 찾는 것입니다. 
  + SVD : 
    + 특이값분해는 m x n 크기의 데이터 행렬 A를 분해하는 걸 말합니다.
    + 고유값 분해(Decomposition)는 변환 후 벡터가 변환 전 벡터에서 얼마나 확대, 축소를 가했는지를 파악하기 위함이다.
  + PCA : 
    + 데이터의 분산(variance)을 최대한 보존하면서 PCA는 보통 데이터의 분포에 대한 주성분 즉, 최적의 Feature 조합을 찾기 위한 방법
...
+ Corner Detection (Harris)
  ...
  + SVD (Singular value decomposition)
    ...
    + A*e = ramda *e 의 방정식을 풀면 det(A-ramda*I) = 0 
      ...
      +  두 개 고유값 중에 더 큰 고유값과 연결된 고유벡터 방향으로 이미지의 변화가 가장 큽니다. 
      +  작은 고유값과 연결된 고유벡터 방향으로의 이미지 변화는 작습니다. 
      +  두 방향으로 변화가 크면(코너점이면), M의 고유값들은 충분히 큰 값들을 갖습니다. 
      +  하나의 고유값은 크고 다른 고유값은 작다면 엣지입니다. 둘 다 0에 가까운 값을 가지면 flat 입니다.  
...
+ SIFT
  ...
  + 특징 기술자 추출
    + Dominant한 방향 결정
    + 그래디언트 히스토그램
    + 특징점 주위에서 4*4 크기의 영역을 n2개 (n*n) 생성
    + 각 영역에 대해 8-방향 계산하고 (이때 특징점 방향을 기준으로 계산, 방향 불변성 달성) 히스토그램 구함 
